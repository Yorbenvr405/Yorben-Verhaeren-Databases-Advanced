...
try: # sommethimes there are some errors while scraping fe the tool got no data
    ...
    while counter < min_input:
        ...
        print("Scraping " + str(counter + 1) + ".") # Logging that there has been looped a loop
        
        counter = counter + 1  # going to next minute for scraper

        if counter < min_input: # if it has runt all the hashes the users want the tool does not have to wait 60 seconds again
    ...
    df_from_redis = r.lrange("data", 0, min_input) # Getting all data from Redis server

    for i in range(0, min_input): # converting byte to dict
        dict_str = df_from_redis[i].decode("UTF-8") # decoding byte
        decoded_data = ast.literal_eval(dict_str) # setting the dict

        df_from_redis[i] = decoded_data # replacing the byte with the dict
    ...
    insert_data = col_bitcoin.insert_many(df_from_redis) # inserting the data

except:
    print("Sommething went wrong please try again.")